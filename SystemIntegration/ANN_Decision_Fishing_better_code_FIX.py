...
dataset = pd.read_excel('datasetFIX_ann_decision_fishh.xlsx')
...
def build_classifier():
    ...
    classifier_kfold.add(Dense(units = 5, kernel_initializer = 'uniform', activation = 'relu', input_dim = 5))
    classifier_kfold.add(Dropout(p = 0.5))
...
# replace 'a' in code below with casting speed value input
# replace 'b' in code below with towing speed value input
# replace 'c' in code below with hauling speed value input
# replace 'd' in code below with dlat value input
# replace 'e' in code below with dlong value input
new_prediction = classifier.predict(sc.transform(np.array([[a, b, c, d, e]])))
