...
# Predictor for predicting missing longitude and latitude position data of illegal transhipment ships
...
training_set = dataset_train.iloc[:10, 1:3].values
...
actual_data = dataset_test.iloc[10:, 1:3].values

inputs = dataset_test.iloc[8:, 1:3].values
...
predicted_longlat_trans = regressor.predict(X_test)
predicted_longlat_trans = sc.inverse_transform(predicted_longlat_trans)

# Evaluating
import math
from sklearn.metrics import mean_squared_error
rmse = math.sqrt(mean_squared_error(actual_data, predicted_longlat_trans))
mape_longlat = np.abs((actual_data - predicted_longlat_trans) / actual_data).mean(axis=0) * 100
